...
std::cout << "\n Queue<Item>::DoEnqueue\n";
  ...
  bool found1 = item->RemovePacketTag(tagCopy);
  double  t =tagCopy.Get();
  
  std::cout << "SNR tag vlaue in tgag= "<< t << "\n";
/*  MyTag tagCopy;
  bool found1 = item->RemovePacketTag(tagCopy);
  
  int  t =tagCopy.GetSimpleValue();
std::cout << "My tag vlaue in tgag= "<< t << "\n";*/




  
  //Ptr<Item> aCopy = item->Copy ();

  // read the tag from the packet copy
  
 // item->PeekPacketTag (tagCopy);
//uint8_t  t = item->GetSimpleValue();
